...
window.UI = {
    ...
    bindSliders: function() {
        ...
        ['dist', 'cutoff', 'res', 'env', 'dec'].forEach(p => {
            ...
            if(el) el.oninput = (e) => update(this.mapParamName(p), parseFloat(e.target.value));
        });
    },
    ...
    mapParamName: function(short) {
        return { 'dist': 'distortion', 'cutoff': 'cutoff', 'res': 'resonance', 'env': 'envMod', 'dec': 'decay' }[short];
    },
    ...
    startLoop: function() {
        const loop = () => {
            ...
            if(window.AppState.isPlaying) {
                this.drawFrame = requestAnimationFrame(loop);
            }
        };
        loop();
    },
    ...
    renderDrumEditor: function() {
        ...
        const kits = window.AudioEngine.drums ? window.AudioEngine.drums.kits : [];
